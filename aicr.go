...
//
// External projects (e.g. the NVIDIA Crossplane provider,
// `provider-nvidia`) should import THIS package and use the types and
...
// clobbering each other — a property the gpucluster controller in
// the NVIDIA Crossplane provider relies on (one Client per
// ProviderConfig). This is a v0.12+ guarantee; earlier facade
// builds mutated a process-global DataProvider via
// recipe.SetDataProvider and were unsafe to construct concurrently.
//
...
// Clients (the NVIDIA Crossplane provider does this by hashing the
// ProviderConfig content) rather than constructing one per request.
//
...
func (c *Client) BundleComponents(ctx context.Context, r *RecipeResult) ([]ComponentBundle, error) {
	...
	for i := range r.Components {
		...
		// successfully. provider-nvidia's emit code accepts both
		// forms, so the AICR contract intentionally matches.
		...
		case "helm":
			values, err := r.internal.GetValuesForComponentWithProvider(dp, facade.Name)
			if err != nil {
			...
			manifests, err := loadManifestFiles(r.internal, dp, facade.Name)
			if err != nil {
				return bundles, err
			}
...
func (c *Client) ValidateState(
	...
	// ValidateOption is a transparent alias of validator.Option, so
	// the variadic args pass through without translation. The slice
	// is rebuilt only to give the underlying constructor the right
	// type signature; no per-option transformation is required.
	...
	return v.ValidatePhases(ctx, nil, recipe.internal, snap)
}
...
func loadManifestFiles(internal *recipe.RecipeResult, dp recipe.DataProvider, componentName string) ([]byte, error) {
