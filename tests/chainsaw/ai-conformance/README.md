...
Validates that the NVIDIA AI-conformance inference stack is correctly generated by Eidos and healthy on a production EKS cluster. The stack satisfies CNCF AI Conformance requirements for GPU scheduling (KAI Scheduler), inference routing (kgateway with Gateway API Inference Extension), and the NVIDIA Dynamo serving platform.
...
- Built eidos binary (`go build -o dist/e2e/eidos ./cmd/eidos`)
...
EIDOS_BIN=$(pwd)/dist/e2e/eidos chainsaw test \
  --no-cluster \
