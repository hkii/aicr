...
func TestNewClientRejectsMissingFilesystemDir(t *testing.T) {
	...
	// /nonexistent/path must not exist on any reasonable CI host; the
	// layered data provider constructor should refuse it rather than
	// silently fall through to the embedded default (which was the
	// bug that motivated this test).
	_, err := aicr.NewClient(
		aicr.WithRecipeSource(aicr.FilesystemSource("/nonexistent/path/provider-nvidia-aicr-test")),
	)
	if err == nil {
...
// path is the one a real provider-nvidia controller would hit, and
// it's the one most likely to regress if a future refactor reaches
// for the package-global DataProvider.
