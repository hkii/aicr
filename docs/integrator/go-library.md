...
The flagship external consumer today is [`provider-nvidia`][provider],
the NVIDIA Crossplane provider. Other integrators with similar needs
(custom admission webhooks, controllers, deployment orchestrators)
should follow the same pattern.

[provider]: https://github.com/NVIDIA/provider-nvidia
...
| OCI registry | `aicr.OCISource(registry, tag)` | **Reserved — not yet implemented.** `NewClient` returns `ErrCodeUnavailable` when this source is selected. Tracked for a future minor release. |

Additional sources (HTTP, Git) may be added in future minor releases.
...
context cancellation. Callers should pass a context with a deadline —
internally the builder applies `defaults.RecipeBuildTimeout` as a cap.
...
- [`provider-nvidia`][provider] — reference external consumer
